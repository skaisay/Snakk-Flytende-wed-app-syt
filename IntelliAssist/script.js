...
window.showSourcesModal = function() {
    ...
    // Получаем статистику ТОЛЬКО из мастер-базы
    const stats = typeof MASTER_DATABASE !== 'undefined' ? {
        translations: Object.keys(MASTER_DATABASE.translations || {}).length,
        conversations: Object.keys(MASTER_DATABASE.conversations || {}).length,
        norwegian: Object.keys(MASTER_DATABASE.norwegian || {}).length
    } : { translations: 0, conversations: 0, norwegian: 0 };
    
    const totalRecords = stats.translations + stats.conversations + stats.norwegian;
    ...
    content.innerHTML = `
        ...
        <div style="margin-bottom: 20px; font-size: 14px;">
            <div style="background: rgba(79,156,249,0.1); padding: 15px; border-radius: 10px; margin-bottom: 15px;">
                ...
                <p style="margin: 5px 0;">• Переводы: ${stats.translations.toLocaleString()} записей</p>
                <p style="margin: 5px 0;">• Разговорные фразы: ${stats.conversations.toLocaleString()} записей</p>
                <p style="margin: 5px 0;">• Норвежская лексика: ${stats.norwegian.toLocaleString()} записей</p>
            </div>
        </div>
        <div style="margin-bottom: 20px; font-size: 14px;">
...
class WebAssistant {
    ...
    /**
     * АВТОМАТИЧЕСКИЙ поиск в мастер-базе данных
     * Работает с ЛЮБОЙ структурой данных в MASTER_DATABASE
     * НЕ ТРЕБУЕТ обновления при добавлении новых категорий!
     */
    searchMasterDatabase(query) {
        ...
        // АВТОМАТИЧЕСКИЙ поиск по всей структуре MASTER_DATABASE
        return this.searchInObject(MASTER_DATABASE, normalizedQuery, 'мастер-база');
    }

    /**
     * УНИВЕРСАЛЬНАЯ рекурсивная функция поиска
     * Автоматически обрабатывает любую структуру данных
     */
    searchInObject(obj, query, sourcePath = '') {
        // Прямое совпадение ключа
        if (obj[query]) {
            return this.formatAnswer(obj[query], query, sourcePath);
        }

        // Поиск по всем ключам и значениям
        for (const [key, value] of Object.entries(obj)) {
            const currentPath = sourcePath ? `${sourcePath}/${key}` : key;
            
            // Если ключ содержит поисковый запрос
            if (key.toLowerCase().includes(query)) {
                return this.formatAnswer(value, key, currentPath);
            }
            
            // Если значение - объект, ищем рекурсивно
            if (typeof value === 'object' && value !== null && !Array.isArray(value)) {
                const result = this.searchInObject(value, query, currentPath);
                if (result) return result;
            }
            
            // Если значение - строка и содержит запрос  
     ...
     * Определяет тип данных и форматирует соответственно
     */
    formatAnswer(value, key, source) {
        // Массив ответов - выбираем случайный
        if (Array.isArray(value)) {
            const randomIndex = Math.floor(Math.random() * value.length);
            return {
                type: 'conversation',
                answer: value[randomIndex],
                source: source
            };
        }
        
        // Объект с переводом и примерами (норвежская лексика)
        if (typeof value === 'object' && value.translation) {
            let answer = `${key} - ${value.translation}`;
            if (value.level) answer += ` (${value.level})`;
            if (value.example) {
                answer += `\nПример: ${value.example}`;
                if (value.exampleTranslation) {
                    answer += `\nПеревод: ${value.exampleTranslation}`;
                }
            }
            return {
                type: 'norwegian',
        ...
        if (typeof value === 'string') {
            return {
                type: 'translation',
                answer: source.includes('translations') 
                    ? `"${key}" по-норвежски: ${value}`
                    : value,
                source: source
            };
        }
        ...
        if (typeof value === 'object') {
            return this.searchInObject(value, key, source);
        }
    ...
    async findBestAnswer(userMessage) {
        ...
        try {
            ...
            // Поиск ТОЛЬКО в мастер-базе данных
            results = this.searchMasterDatabase(userMessage);
            const searchTime = performance.now() - searchStart;
            console.log(`🔍 Поиск в мастер-базе выполнен за ${searchTime.toFixed(2)}мс`);
            
            // Обрабатываем результаты поиска из мастер-базы
            if (results) {
                if (results.type === 'translation') {
                    return results.answer;
                } else if (results.type === 'conversation') {
                    return results.answer;
                } else if (results.type === 'norwegian') {
                    return results.answer;
